...
(function() {
				...
				plus.gallery.pick(function(e) {
					...
					plus.zip.compressImage({
						...
						if(size > (10 * 1024 * 1024)) {
							return mui.toast('文件总大小超过10M,请重新选择~');
						}
		...
		feedback.submitBtn.addEventListener('tap', function(event) {
			if(feedback.question.value == '') {
				return mui.toast('问题和意见填写不符合规范');
			}
			//		if(feedback.contact.value == '') {
			//			return mui.toast('联系方式填写不规范');
			//		}
		...
		feedback.send = function(content) {
			//console.log("content===" + content.score + "contact=" + content.images.length);
			//return;
			var imgFiles;
			if(fileUploadList != null && fileUploadList != 0) {
				console.log("===开始"+fileUploadList.length);
				//plus.nativeUI.showWaiting("正在上传图片...");
				for(var i = 0; i < content.images.length; i++) {
					var fileObjs = content.images[0]; 
				}
		}else{
		}
		...
		var idea = new Idea();
		idea.set("content", content.content);
		idea.set("phone_", content.contact);
		idea.set("score", content.score);
		//idea.set("imgFiles", content.images);
		idea.save(null, {
			success: function(ideaResult) {
				plus.nativeUI.closeWaiting();
				mui.alert("感谢反馈，我们会认真查看并回复。(点击确定关闭)", "问题反馈", "确定", function() {
					feedback.clearForm();
					mui.back();
				});
			...
			error: function(ideaResult, error) { 
				plus.nativeUI.closeWaiting();
				// 添加失败
				alert('添加数据失败，原因：' + error.description);
			}
		});
